...
class Room:
    def __init__(self, opt):
        ...
        try:
            # Create recv socket
            self.recvsock = socket.socket( socket.AF_INET6, socket.SOCK_DGRAM, socket.IPPROTO_UDP)
            ...
            mreq = socket.inet_pton(socket.AF_INET6, self.room)
            mreq += struct.pack("@I", 0) 
            print mreq
            print self.room
            # join the group
            self.recvsock.setsockopt( socket.IPPROTO_IPV6, socket.IPV6_JOIN_GROUP, mreq)
            ...
            self.sendsock = socket.socket( socket.AF_INET6, socket.SOCK_DGRAM, socket.IPPROTO_UDP)
